...
SUNDIALS_EXPORT int CVodeReInitSuperLUMT(void *cvode_mem)
{
	CVodeMem cv_mem;

	if (cvode_mem == NULL) {
		cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeReInitSuperLUMT", MSGCV_NO_MEM);
		return(CV_MEM_NULL);
	}

	cv_mem = (CVodeMem)cvode_mem;

	if (cv_mem->cv_lreinit)
		cv_mem->cv_lreinit(cv_mem);

	return(CV_SUCCESS);
}

SUNDIALS_EXPORT int CVodeFreeSuperLUMTGlobalData(void *cvode_mem)
{
	CVodeMem cv_mem;

	if (cvode_mem == NULL) {
		cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeFreeSuperLUMTGlobalData", MSGCV_NO_MEM);
		return(CV_MEM_NULL);
	}

	cv_mem = (CVodeMem)cvode_mem;

	if (cv_mem->cv_lfreeglobal)
		cv_mem->cv_lfreeglobal(cv_mem);
