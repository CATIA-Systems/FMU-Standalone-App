...
extern void delayBuffersCloseNew(struct DYNInstanceData*, int destructExternalObject);
...
DYMOLA_STATIC FMIStatus fmiFreeFMUstate_(FMIComponent c, FMIFMUstate* FMUstate)
{
	...
	if (target->did) {
		/* free external object iff external object either not reference counted or reference count reached 0 */
		delayBuffersCloseNew(target->did, util_handle_external_object_ref_count(dynExternalObjectFirst(target->did), -1) <= 0);
	}
...
DYMOLA_STATIC FMIStatus fmiGetFMUstate_(FMIComponent c, FMIFMUstate* FMUstate)
{
	...
	if (*FMUstate == NULL) {
		...
		Component* curStateComp = (Component*)*FMUstate;
		/* reference count external object to avoid premature destruction */
		if (util_handle_external_object_ref_count(dynExternalObjectFirst(curStateComp->did), -1) == 0) {
			/* external object reference count reached 0 */
			delayBuffersCloseNew(curStateComp->did, 1);
		}
	}
...
DYMOLA_STATIC FMIStatus fmiSetFMUstate_(FMIComponent c, FMIFMUstate FMUstate)
{
	...
	/* set state vectors, derivatives, etc */
	{
		int ret, test = 0;
		/* reference count external object to avoid premature destruction */
		if (target->mStatus > modelInstantiated && (test=util_handle_external_object_ref_count(dynExternalObjectFirst(target->did), -1)) <= 0) {
			/* external object reference count either not reference counted or reference count reached 0  */
			delayBuffersCloseNew(target->did, 1);
		}
												...
												FMIFMUstate* FMUstate)
{
	...
	if (tpl_load(tn, TPL_MEM, serializedState, size) != 0) {
		...
		if (tpl_unpack(tn, 0) < 0) {
			...
			/* decrement reference count for current state since it is overwritten */
			if (util_handle_external_object_ref_count(dynExternalObjectFirst(state->did), -1) == 0) {
				/* external object reference count reached 0 */
				delayBuffersCloseNew(state->did, 1);
			}
			/* restore did */
			...
			/* increment reference count for new state */
			util_handle_external_object_ref_count(dynExternalObjectFirst(state->did), 1);
		}
...
static int copyVariables(Component* source, Component* target, const FMICallbackFunctions* funcs)
{
  ...
  if(target->did && dyn_allowMultipleInstances && source->did){
	  ...
	  util_handle_external_object_ref_count(dynExternalObjectFirst(source->did), 1);
