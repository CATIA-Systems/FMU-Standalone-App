...
DYMOLA_STATIC int util_handle_external_object_ref_count(void* objAddr, int count)
{
	static UtilExternalObjRefCount refCounts[MAX_EXTERNAL_OBJECTS_REF_COUNTERS];
	UtilExternalObjRefCount* rCp = NULL;
	static int nextIx = 0;
	int i;

	if (objAddr == NULL) {
		/* convenient shortcut for caller */
		return -1;
	}

	/* check if it already exists */
	for (i = 0; i < nextIx; i++) {
		if (refCounts[i].objAddr == objAddr) {
			rCp = &refCounts[i];
			break;
		}
	}
	...
	/* non-existing */
	if (count <= 0) {
		/* requested decrement or just check */
		return -1;
	}

	/* increment requested, need to create new */
	if (nextIx == MAX_EXTERNAL_OBJECTS_REF_COUNTERS) {
		/* cannot log here since the provided comp may not be sufficiently setup for that */
		return -1;
	}

	refCounts[nextIx].objAddr = objAddr;
	/* for new, referece count is always 2; original and first copy */
	refCounts[nextIx].count = 2;
	++nextIx;
	return count;
}
