...
/* Handle reference counting for external objects */
/* Use cases: 
- Add new or increment: count > 0. For new reference counter, count will be set to 2 (for original and first copy)
- Decrement if exists: count < 0.
- Just check for existence: count = 0.
Note: If objAddr == NULL, -1 is returned and nothing is performed.
*/
/* Returns: count after update if exists or could be created, else -1 */
